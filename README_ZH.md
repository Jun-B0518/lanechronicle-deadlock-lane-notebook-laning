...
- **分路复盘卡片** (Lane review cards) — 规划模块，尚未实现。
- **物品时机工作表** (Item timing worksheets) — 规划模块，尚未实现。
- **补丁注释日志** (Patch annotation log) — 规划模块，尚未实现。
...
仅使用手工输入和获得用户授权的会话后数据。不读取游戏内存，不获取隐藏的对手信息，不自动输入，不修改游戏客户端。
...
- [steam-chart-page](https://store.steampowered.com/charts/mostplayed) — 该产品出现在保存的 Steam 活跃游戏页面中。 (2026-09-16T14:21:03.7732730Z)
